...
import {Initializable} from "libr/oz/Initializable.sol";

import {ABDKMath64x64} from "libr/abdkMath/ABDKMath64x64.sol";
...
abstract contract OsmoticFormula is Initializable {
    ...
    /**
     * @custom:mod
     * Aca vamos a cambiar todos los valores por uints enteros, dejando como uso los primeros 192 bits como el numero y 64 bits como decimales
     */
    int128 public decay;
    int128 public drop;
     ...
     */
    function __OsmoticFormula_init(
    ...
    function minStake(uint256 _totalStaked) public view returns (uint256) {
         ...
         * @custom:modificacion
         *
         * Esto no deberia ser un numero fijo sino que se use en terminos porcentuales, de esa forma no se fija a una cantidad x sino a una relacion entr x e y
         */
    ...
    ) public view returns (uint256 _targetRate) {
        if (_stake == 0) {
            _targetRate = 0;
        } else {
            ...
            /**
             * @custom:mod
             * si _stake es menor a _minStake retornar 0, asi no gastamos computacion al pedo
            _targetRate = _stake > _minStake
                ? (
                    ONE.sub(
                        _minStake
                            .divu(_stake)
